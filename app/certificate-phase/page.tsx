  ...
  async function elementsCreate() {
    try {
      ...
      const inputSelectString: string =
        'あなたは，与えられたデータからセキュリティ質問として適切で強固な，質問と回答のセットを5つ選定する人です．出力は "[{"question":["質問1"],"answer":["回答1"]},{"question":["質問2"],"answer":["回答2"]}]" のようなJSON形式のみ許可します．また，**絶対に同じような質問(question)は含めてないでください**．回答が「わからない」「覚えてない」などの求めている回答になっていないものや，個人の意見や感想に基づくものは、スキップして出力しないでください。ペットの質問などで，「いない」といった回答は採用しても良いです．感じたことや気持ちを含むものは含めないでください．ハルシネーションを起こさないように慎重に出力してください．**入力されたデータ以外を出力しないでください**．**同じトピックの質問を繰り返さないで下さい**．ハルシネーションを起こさないで，入力されたデータから変更しないでださい．';
